  ...
  return (
    <>
      ...
      >
        ...
        <Description className="-mt-1.5 mb-1">
          Define se o resultado confirmado entre os jogadores já vale ou precisa
          da aprovação do admin.
        </Description>
        <Segment
          isDisabled={isDisabled}
...
const ResultRulesSection = ({ isDisabled }: RuleSectionProps) => {
  ...
  return (
    <>
      <TextField isInvalid={Boolean(errors.ruleConfig?.winBehavior)} isRequired>
        ...
        <FieldError>{errors.ruleConfig?.winBehavior?.message ?? ""}</FieldError>
      </TextField>
...
export const Rules = ({ isDisabled: isDisabledProp, isLocked }: RulesProps) => {
  const [activeTab, setActiveTab] = useState("desafios");
  const isDisabled = isDisabledProp || isLocked;
