...
- **Como funciona:** jogador ou organizador submete `{sets:[0-0], walkover:true, winnerMembershipId}` → valida `resolveWalkoverScoreError` (vencedor ∈ lados, 1 set zerado, behavior não-cancelamento) em vez de `validateChallengeScore` → fluxo de confirmação normal com notificações `walkover_submitted`/`walkover_confirmed`. Edição pelo organizador: novo submission confirmado + restore/re-apply de ranking + auditoria + `result_edited`. Delta de UI: **chip "W.O." do card padrão ENTREGUE** (o item da agenda já chega com `matchStatus: "walkover"`, traduzido pelo `resolveLeagueScheduleOutcome` — o card padrão é quem desenha o chip); o que segue pendente é a **pintura do vencedor do W.O. na agenda da liga**, que depende do contrato: o item do `listScheduled` não carrega o lado vencedor, então o card fica com o chip W.O. e SEM placar (o `0x0` do placeholder é descartado) e sem pintura, até o contrato expor o vencedor.

### Agenda / Schedule
- **Status:** implementado
- **Data:** 26/06/2026 (agenda); 23/09/2026 (status e placar no card)
- **Referências:** `convex/domains/league/contract.ts` — `LeagueScheduleVisibilityOptions` (:57-60), `DEFAULT_LEAGUE_SCHEDULE_VISIBILITY = "public"` (:64), campo `scheduleVisibility` no `ChallengeRuleConfigSchema` (:504), `leagueScheduleItemSchema` (:926-941) com `matchStatus` (`LeagueScheduleMatchStatusOptions`, :896-901) e `scoreSets` (`leagueScheduleScoreSetSchema`, :910-920), e o resolvedor `resolveLeagueScheduleOutcome` (:949-984). `convex/domains/league/challenge-scheduling-rules.ts` — `isChallengeVisibleOnSchedule` (:63-65). `convex/functions/league/challenges.ts` — `league.challenges.listScheduled` (`authQuery`, :167-265): se `scheduleVisibility !== "public"` exige `getViewerContextOrThrow` (FORBIDDEN para visitante); publica `confirmed`+`finished` com `matchDate >= hoje` (:187-198); ordena por `matchDate`/`startMinute` (:257-264). `src/components/ui/match-card.tsx` (GLOBAL — RUL-0005; substitui o `schedule-card.tsx`, extinto) — card das duas agendas, com chip de status (`matchStatus?`, :99) e placar (`scoreSets?`, :115). `src/app/(private)/leagues/[leagueId]/schedule.tsx` — rota standalone (Page.Header + ScrollView), janela `7|15 dias`, tabs de data via `buildScheduleDateTabs`, sempre abre em "Hoje", períodos manhã/tarde/noite via `buildScheduleDayView`, e monta o `MatchCard` com `matchStatus`/`scoreSets` do item (:165-176). `src/app/(private)/leagues/[leagueId]/index.tsx` — item "Agenda" no menu `⋮` quando `access.canOpenSchedule` (:476-485). `src/lib/leagues/league-details-derived.ts` — `canOpenSchedule: scheduleVisibility === "public" ? true : isMember` (:108). `src/lib/leagues/schedule-view.ts` — `buildScheduleDateTabs`, `buildScheduleDayView` (:98-129; manhã <720, tarde <1080, noite >=1080), `SCHEDULE_WINDOW_OPTIONS` (7/15), `SCHEDULE_PERIOD_META`, `formatScheduleMinute`; testes em `schedule-view.test.ts`. `settings.tsx` — "Visibilidade da agenda" (`scheduleVisibilityOptions`: "Aberta para todos"/"Somente jogadores").
- **Status e placar no item (vocabulário do card padrão das agendas):** `matchStatus` usa o MESMO vocabulário do chip do chaveamento (`getMatchStatusChip`, `src/lib/tournaments/tournament-details-derived.ts:145-151`), sem o `vacant` da chave: `scheduled` ("Agendado"), `finished` ("Encerrado"), `pending` ("A definir") e `walkover` ("W.O."). O status cru do desafio não vaza para o contrato — o card imprimiria o enum. `resolveLeagueScheduleOutcome` traduz: jogo liquidado → `finished` (`walkover` quando o score carrega a flag), `confirmed` → `scheduled`, qualquer outro → `pending` (o filtro da agenda não deixa `pending` passar; o valor existe no vocabulário compartilhado do chip). `scoreSets` é `null` fora do liquidado e, no liquidado, a lista no vocabulário A/B do card (challenger = lado A, challenged = lado B) — cada set é `{aGames, bGames, kind: "set"|"tiebreak"|"super_tiebreak", tieBreak: {aPoints, bPoints} | null}`; placar liquidado sem sets serializa `null`.
- **Visibilidade da agenda:** `isChallengeVisibleOnSchedule` publica `confirmed` e `finished`. Resultado ainda em CONFIRMAÇÃO não entra e não publica placar na agenda: ela é pública e o `listForLeague` esconde esse resultado de quem não joga — só o liquidado ganha placar no item. O corte `matchDate >= hoje` continua valendo (um `finished` aparece enquanto o dia não passou).
- **Decisões:** filtro de janela client-side; flag vive no `ruleConfig` com fallback `?? "public"` no backend e no client; serialização com fallback sem migração de dados (docs legados caem no default). A agenda da liga reusa o MESMO card do torneio em vez de manter um card próprio de leitura reduzida.
...
- **BUG-0042 (20/09/2026):** o aviso do suspenso no `GuestOverview` caiu junto com o card de features no corte de conteúdo do IBX-0071 e a página ficou MUDA para o membro suspenso (que segue no papel guest). Restaurado: `buildLeagueGuestOverviewAlert({ now, reminderDaysBefore, status })` isola o único sub-estado de visitante que tem o que avisar — `suspended` → o mesmo alerta danger do `buildLeaguePaymentAlert`, sem CTA dentro do alerta — e o `GuestOverview` o renderiza ANTES da descrição da liga. `payment_due` e `active` continuam fora daqui (papel de membro, aviso no `PlayerOverview`: nunca dois avisos da mesma pendência). O caminho da ação é o rodapé de entrada: o suspenso TEM `viewerMembershipId` no `discovery.getById` (o servidor devolve a membership sem filtrar por status, `convex/functions/league/discovery.ts:158-166`), então `canResumeCheckout` → CTA habilitado → `handlePayPress` (charge PENDING em fast-path ou `createCharge`) → `/checkout/[chargeId]`, onde o `canRenew` do checkout cobre o suspenso (`src/lib/payments/checkout-view.ts:78,119`). **(SUPERSEDE no IBX-0084, 21/09: o caminho da ação saiu do rodapé — o CTA `Renovar` vive no item do servidor e o rodapé de entrada não monta para o suspenso; ver os dois bullets abaixo.)**
- **Cutover (20/09/2026, Etapa 2 do PLN-0008):** `buildLeaguePaymentAlert` e `buildLeagueGuestOverviewAlert` foram **EXTINTOS** — os avisos da casa da liga (jogador E visitante) vêm do item do servidor (`pendings.list` do escopo player, recortado pela liga/membership no bucket) pelo renderer único `PendingAlerts`, e o CTA de pagamento do item é a MESMA `payment.charge.createCharge` (`action: pay_league_membership`). O CTA de renovar do suspenso no rodapé segue igual **(SUPERSEDE no IBX-0084, 21/09: o CTA passou para o próprio item do servidor e o rodapé do suspenso deixou de montar — ver o bullet abaixo)**. Ver `docs/spec/dashboard.md`, seção "Pendências em tela — Etapa 2".
- **Corte do pagamento duplicado do suspenso (21/09/2026, IBX-0084):** o item `player_league_membership_suspended` do SERVIDOR passou a carregar o CTA (`actionLabel: "Renovar"` + `action: pay_league_membership`, a MESMA ação dos kinds 1 e 2 — o `createCharge` já aceita `suspended`; `convex/domains/payment/pendings-rules.ts:118-140`) e a casa da liga deixa de duplicar o pagamento do suspenso no rodapé de entrada: o BUG-0042 (dois botões de pagamento para a mesma membership na MESMA tela) passa a ser respeitado pelo caminho inverso — a ação vive no item, que é o único renderizado pelas 6 montagens de `PendingAlerts` em `src/` (`pages/home/player-dashboard.tsx:63`, `pages/home/organizer-dashboard.tsx:52`, `pages/leagues/guest-overview.tsx:35`, `pages/leagues/player-overview.tsx:63`, `pages/tournaments/player-overview.tsx:93` e `pages/tournaments/organizer-overview.tsx:63`), e a tela só renderiza.
  - **Frontend (mesmo card):** o suspenso sai dos DOIS caminhos de pagamento do rodapé — `buildLeagueDetailsCanResumeCheckout` (`src/lib/leagues/league-details-derived.ts:175-179`) cobre só `awaiting_payment` e `buildLeagueDetailsShowJoinFooter` (`:194-204`, agora com TRÊS inputs — `canJoinLeagues` + `role` + `viewerMembershipStatus`, alimentados no bucket por `src/lib/leagues/league-details-store.ts:298-303`) **não monta o rodapé para o membro suspenso**: ele cai no papel guest (segue guest, `buildLeagueDetailsRole`) e o estado dele é o único do papel guest que tem a ação em outra superfície. **Correção da 2ª passada (MEDIO-1 do review):** o gate de CAPACIDADE voltou ao rodapé — a assinatura mantém `canJoinLeagues?` e a derivada FALHA FECHADO (`canJoinLeagues === true`), então o ator organização (sem a capacidade) não tem rodapé em NENHUM estado, como no HEAD. Paridade provada por script (matriz status × ator, HEAD × árvore): a árvore difere do HEAD em EXATAMENTE um caso — o ator que PODE entrar no estado `suspended` (HEAD `true` → agora `false`); nos outros 26 pares (9 status × 3 atores) os valores são idênticos. A rota passa a consumir a derivada como gate (`src/app/(private)/leagues/[leagueId]/index.tsx:229` e `:545`, no lugar do `role === "guest"` da seção IBX-0074 r24 — **SUPERSEDE aquela linha do gate**, o resto do r24 vale). O CTA do alerta **não tem handler na tela**: quem executa é o runner compartilhado (`src/lib/pendings/use-pending-action-runner.ts:323-328`, `pay_membership` → `payment.charge.createCharge` + `/checkout/[chargeId]`), o mesmo dos kinds 1 e 2 — o alerta já desenhava o botão sempre que o item tem `actionLabel` + ação resolvível (`src/components/ui/pending-alerts.tsx:79-96`, o gate em `:88`). O pre-fetch da charge PENDING da tela (`getPendingCharge`) perde o `suspended` da lista de `enabled`: ele só alimenta o fast-path do rodapé (`handlePayPress`), que não existe para esse estado.
  - **Tabela de estado do rodapé guest (antes → depois), por status da membership:** `sem membership` = "Solicitar entrada" (ou "Sem vagas") → **intocado**; `pending` = "Pendente" + X de cancelar, CTA desabilitado → **intocado**; `awaiting_payment` = "Pagar inscrição" + X de cancelar, CTA habilitado (checkout) → **intocado**; `rejected` = "Solicitar novamente" → **intocado**; `left`/`removed` = "Solicitar entrada" → **intocados**; `suspended` = **antes** "Renovar inscrição" com CTA habilitado no `handlePayPress` → **depois** rodapé FORA, com o `Renovar` (uma palavra, `actionLabel` do servidor) como botão do alerta; `payment_due`/`active` = papel de membro, rodapé nunca montou → **intocados** (o CTA do `payment_due` é o do alerta do membro, IBX-0039). No eixo do ATOR (2ª passada): o rodapé de quem NÃO pode entrar (`canJoinLeagues` false) não monta em nenhum dos 9 estados — igual ao HEAD.
  - **Rótulo morto apagado (BAIXO-1 do review, 2ª passada):** o ramo `suspended` de `getMembershipActionLabel` (`src/lib/leagues/presentation.ts`) foi REMOVIDO — sem o rodapé do suspenso ele não tinha mais consumidor e contradizia o estado final (o servidor manda `Renovar`). A cadeia cai no fallback de sempre (`isLeagueOrganizer ? "Entrar como jogador" : "Solicitar entrada"`), que é o valor que os outros estados sem rótulo próprio já usam; nenhum outro ramo foi tocado.
  - **Testes:** `src/lib/leagues/league-details-derived.test.ts` (o `suspended` deixa de ser atalho de checkout; o rodapé por status e por CAPACIDADE — todos os status com `canJoinLeagues: false`, e o fail-closed sem o input) e `src/lib/leagues/league-details-store.test.ts:276-331` (o par capacidade × status no bucket: ator organização → 0 de 9 estados; com a capacidade → todos menos `suspended`, com `active`/`payment_due` fora por papel de membro). Ver `docs/spec/pendings.md` ("Decisões e apontamentos").
...
- **Status:** nao implementado (fora de escopo mantido) — o agendamento usa os slots escolhidos manualmente nas propostas, com checagem de conflito (`listOccupiedSlots` — `convex/functions/league/challenges.ts:267`; `isChallengeSlotBlocked` — `convex/domains/league/challenge-rules.ts:488`).
