...
- **`tournament-details-store.ts`** — bucket por `tournamentId`
  ...
  shouldFetchMatches/tabItems/screenState). `reset()` zera os DADOS (discovery,
  entries, matches, pendings, viewer), devolve `bootstrapStatus` para
  `loading` e **INCREMENTA**
  ...
  vazio ao voltar para um torneio já visitado). O reset NÃO zera a identidade
  da sessão: o ator armado (`actorKey`) e a marca d'água do payload
  (`payloadUpdatedAt`) sobrevivem — é por eles que um re-mount com outro ator
  ainda reconhece o dado do ator anterior.
  O bucket é AMARRADO ao ator ATIVO (`identity.actorKey`): trocar de ator (de
  modo ou de organização) zera o payload e sobe o piso de geração
  (`actorSwitchAt` contra `payloadUpdatedAt`), então dado hidratado sob o ator
  anterior nunca libera a tela. `payloadUpdatedAt` é marca d'água MONOTÔNICA —
  só avança, o carimbo do último payload hidratado nunca regride com um push
  velho — e o piso da troca sai dela, nunca do carimbo da query (num switch com
  a tela montada as duas subscriptions voltam na mesma transição e o carimbo da
  query já é o do ator novo). `derived.screenState` (`error|loading|ready`)
  é a fonte única do estado da tela: `ready` exige descoberta hidratada, ator
  resolvido e payload do ator vigente.
...
- **`_layout.tsx`** — `tournamentId` vem de `useLocalSearchParams` (params
  ...
  carimbo do dado (`dataUpdatedAt`) — um push igual em conteúdo muda só o
  carimbo, não a referência — e erro do contexto do ator vira
...
- **`index.tsx`** (overview) — composição da página (QA round 6, RUL-0007):
  ...
  **Iniciar
  ...
  dois juntos; o menu ⋮ do header também só existe com o papel resolvido —,
...
- **BUG-0045 (aba inicial das Inscrições perdida na entrada FRIA):** o
  ...
  (`lib/tournaments/tournament-details-derived.ts:244-263`): o `pending` passa a
  ...
  a aba de entrada sem `initialTab` e para quem não é organizador. Prova: 4
  casos em `tournament-details-derived.test.ts` (frio→quente com
  `initialTab=pending`, sem `initialTab`, jogador, escolha manual).
- **BUG-0047 (parte do torneio):** o toast de recusa de convite repetia a frase
...
## IBX-0087 · Esqueleto no valor dos KPIs da casa do torneio (21-09-2026, sem commit)
